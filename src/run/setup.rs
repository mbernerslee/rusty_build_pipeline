...
pub fn determine(args: Vec<String>) -> Result<(Setup, Vec<BuildStep>), String> {
    ...
    build_step_validator::validate(&build_steps)?;
...
mod test {
    mod determine {
        ...
        fn errors_when_the_config_is_valid_json_but_invalid_for_another_reason() {
            ...
            let error = String::from("Giving up because build_step_name 'sayHello' depends on 'noExistey', but that doesn't exist!");
